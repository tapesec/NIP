...
<div class="wall_reply">
	<table class="table table-striped table-bordered table-hover table-condensed">
		<tr>
			<?php echo '<td colspan="2"><span class="date"><small>'.DateHelper::fr($post['sub_dateC']).'</span><span class="title"><h4>'.$post['sub_title'].'</small></h4></span>'; ?>
				<?php if(isset(Auth::$session['use_checked']) && Auth::$session['use_checked'] == true): ?>
					...
					<span class="action"><small><a href="<?php echo BASE_URL.'/forum/addSubject/'.$post['sec_id'].'/'.$post['sub_id'];?>">Editer</a></span><span class="action"><a href="<?php echo BASE_URL.'/forum/delSubject/'
					.$post['sub_id'];?>">Supprimer</a></small></span>
					<?php endif; ?>
				<?php endif; ?>
			<?php echo '</td>'; ?>
		</tr>
		<tr>		
			...
			<?php echo '</td>'; ?>
			<?php echo '<td>'.$this->markitup->bbcodeParse($post['sub_content']).'</td>'; ?>
		</tr>
		<?php// debug($posts['list'][0]['rep_content']); ?>
	<?php if(!empty($posts['list'][0]['rep_content'])): ?>
		<?php foreach ($posts['list'] as $k => $v): current($v); ?>

			<tr>
				<?php echo '<td colspan="2">'; ?>
				<?php echo '<span class="date"><small>'.DateHelper::fr($v['rep_dateC']).'</span><span class="title"><h4><small>'.$v['rep_title'].'</small></h4></span>'; ?>
				<?php if(isset(Auth::$session['use_checked']) && Auth::$session['use_checked'] == true): ?>
					<?php if(Auth::$session['use_id'] == $v['rep_id_author'] || Auth::$session['use_statut'] >= 2): ?>
					<?php echo '<span class="action"><a href="'.BASE_URL.'/forum/posts/'.$posts['list'][0]['sub_id'].'/'.$v['rep_id'].'">Editer</a></span><span class="action"><a href="'.BASE_URL.'/forum/delReply/'.$v['rep_id'].'">Supprimer</a></span>'; ?>
					<?php endif; ?>
				<?php endif; ?>
				<?php echo '</td>'; ?>
			</tr>
			<tr>		
				...
				<?php echo '</td>'; ?>
				<?php echo '<td>'; ?>
				<?php echo $this->markitup->bbcodeParse($v['rep_content']); ?>
				<?php if(!empty($v['rep_dateM'])): ?>
				<?php echo '<span class="edited"><small><em>Edité le '.DateHelper::fr($v['rep_dateM']).'</em></small></span>'; ?>
				<?php endif; ?>
				<?php echo '</td>'; ?>
	...
	<?php if(isset(Auth::$session['use_checked']) && Auth::$session['use_checked'] == true): ?>
		...
		<tr>
			...
			<td>
				...
				<?php echo $this->Form->create('forum/posts/'.$post['sub_id'].'/'.$id = (isset($rep['rep_id']))? $rep['rep_id'] : '', array('type' => 'POST', 'name' => 'rep_id', 'value' => $id)); ?>
			
				<?php echo $this->Form->input(array('name' => 'rep_title', 'type' => 'text', 'label' => 'titre : (facultatif)', 'value' => $value = (isset($rep['rep_title']))? $rep['rep_title'] : '', 'message' => true)); ?>
				...
				<?php echo $this->Form->input(array('name' => 'rep_content', 'type' => 'textarea', 'label' => '', 'value' => $value = (isset($rep['rep_content']))? $rep['rep_content'] : '','rows' => 5, 'cols' => 20, 'class' => 'markitup', 'message' => true)); ?>
