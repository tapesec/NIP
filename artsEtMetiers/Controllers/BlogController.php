...
class BlogController extends Controller{
	...
	**/
	public function index($page=1){
		...
		$data = $this->Article->find(array('fields' => 'art_id, art_title, art_content, art_dateC, cat_name, use_login',
									 'where' => array(
									 	'art_slot' => 'blog',
									 	'art_online' => true),
									 'join' => array(
	...
	public function voir($id, $id_com=null, $com_id_user=null){
		...
		if($this->request->is('GET')){
			...
			if($this->Comment->update($this->request->data, array('where' => array('com_id' => $id_com)))){
				$this->session->setFlash('Message bien edité !');
				$this->redirect('blog/voir/'.$id);

			}else{
				$this->session->setFlash('Erreur lors de l\'édition de votre commentaire');
				$this->redirect('blog/voir/'.$id);
			}
			...
			if($this->Comment->save($this->request->data)){
				...
				$this->session->setFlash('Veuillez corriger vos érreurs !');
				$this->redirect('blog/voir/'.$id);
			}
		}
	...
	**/
	public function delete($id){
		...
		if($this->Comment->delete(array('com_id' => $id))){
			$this->session->setFlash('Commentaire bien supprimé');
			$this->redirect($this->referer);
		}else{
			$this->session->setFlash('Impossible de supprimer votre commentaire, ressayez ou contactez l\'administrateur du site');
			$this->redirect($this->referer);
		}
		$this->render('voir');
	}
